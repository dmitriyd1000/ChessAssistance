...
using ChessAssistanceMain.Source.ViewModel;
using ChessDotNet;
    ...
    public partial class FormMain : Form
    {
        ...
        private void OnBestMoveFound(string move)
        {
            if (!_movePlayed)
            {
                try
                {
                    Invoke((Forms.MethodInvoker)delegate { PlayMove(move); });
                }
                catch (Exception ex)
                {
        ...
        private void StartTimeAnalysis(int milliseconds)
        {
            ...
            var analysisTask = new Task(() =>
            {
                for (var i = _vm.NavigationIndex; i < _vm.PlyList.Count; i++)
                {
                    ...
                    Invoke((Forms.MethodInvoker) delegate
                    {
                        Navigate(uiI);
                    ...
                    {
                        Invoke((Forms.MethodInvoker)delegate { dialog.Hide(); });
                        return;
                    }
        ...
        internal void ResetGame(ChessGame newGame)
        {
